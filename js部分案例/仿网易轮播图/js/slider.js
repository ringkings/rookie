...
window.onload = function() {
    ...
    for(var i=0;i<imgs.length; i++) {
        ...
        span.innerHTML = imgs.length-i;  //  6 - 0     6 - 1   // 实现 倒序 的方式插入倒序插入
    ...
    for(var k in spans){   //   k  是索引号  spans[k]    spans[0]  第一个span
        spans[k].onclick = function() {
            ...
            if(this.className == "slider-ctrl-prev"){ // 判断当前点击的这个按钮是不是 prev
                ...
                // alert(this.innerHTML);下面开始做点击底下按钮时候
                // 我们首先要知道我们点击是第几张图片  --- 获得当前的索引号
                var that = this.innerHTML - 1;
                // console.log(typeof that);
                if(that > iNow) {//索引号大于现在张数，说明点击的是右面的
                      // 做法等同于 右侧按钮
                    animate(imgs[iNow],{left: -scrollWidth});  // 当前的这张慢慢的走出去 左侧
                    imgs[that].style.left = scrollWidth + "px"; // 点击的那个索引号 快速走到右 310
                }
                else if(that < iNow) {
                    // 做法等同于 左侧按钮
                    animate(imgs[iNow],{left: scrollWidth});
                ...
                iNow = that;  // 给当前的索引号，没有这句iNow一直是1
                animate(imgs[iNow],{left: 0});//这句话本来放在条件句但是因为二者都有故可以提出来
